...
class User(db.Model, UserMixin):
    ...
    # --- Sistema de Logros ---
    unlocked_achievements_assoc = db.relationship(
        "UserAchievement", back_populates="user"
    )

    __table_args__ = (db.Index("idx_user_level_xp", "level", "xp"),)
    ...
    def send_friend_request(self, user):
        if (
            not self.has_sent_request_to(user)
            and not user.has_sent_request_to(self)
            and not self.is_friend(user)
            and self != user
        ):
    ...
    def has_sent_request_to(self, user):
        return (
            self.sent_requests.filter(friend_request.c.receiver_id == user.id).count()
            > 0
        )

    def has_received_request_from(self, user):
        return (
            self.received_requests.filter(friend_request.c.sender_id == user.id).count()
            > 0
        )
