...
def test_limite_sala_llena(socket_env_limit):
    ...
    assert (
        "llena" in error_msg["mensaje"].lower()
        or "máximo" in error_msg["mensaje"].lower()
    )
...
def test_victoria_por_intercambio(juego_interacciones):
    ...
    res = juego_interacciones.usar_habilidad_jugador(
        p1.nombre, len(p1.habilidades), objetivo=p2.nombre
    )
