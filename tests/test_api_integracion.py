...
def test_login_exitoso(cliente_test):
    ...
    cliente_test.post(
        "/register",
        data=json.dumps(
            {"username": "LoginTester", "email": "login@test.com", "password": "123"}
        ),
...
def test_login_fallido(cliente_test):
    payload = {"email": "noexiste@test.com", "password": "123"}
    response = cliente_test.post(
        "/login", data=json.dumps(payload), content_type="application/json"
    )
...
def test_perfil_usuario(cliente_test):
    ...
    cliente_test.post(
        "/register",
        data=json.dumps(
            {"username": "ProfileUser", "email": "p@p.com", "password": "123"}
        ),
        content_type="application/json",
    )
