...
def test_habilidad_sabotaje(juego_combate):
    ...
    # Usar habilidad
    resultado = juego_combate.usar_habilidad_jugador(atacante.nombre, atacante.habilidades.index(sabotaje) + 1, objetivo=victima.nombre)
...
def test_bomba_energetica_area(juego_combate):
    ...
    resultado = juego_combate.usar_habilidad_jugador(lanzador.nombre, lanzador.habilidades.index(bomba) + 1)
...
def test_intercambio_forzado(juego_combate):
    ...
    intercambio = Habilidad("Intercambio Forzado", "movimiento", "desc", "🔄", 5, COSTO_INTERCAMBIO)
    ...
    resultado = juego_combate.usar_habilidad_jugador(p1.nombre, idx_habilidad, objetivo=p2.nombre)
...
def test_invisibilidad_proteccion(juego_combate):
    ...
    juego_combate.usar_habilidad_jugador(defensor.nombre, defensor.habilidades.index(invis) + 1)
    
    assert any(e['tipo'] == 'invisible' for e in defensor.efectos_activos)
    
    # Atacante intenta usar Sabotaje
    sabotaje = next(h for h in atacante.habilidades if h.nombre == "Sabotaje")
    resultado = juego_combate.usar_habilidad_jugador(atacante.nombre, atacante.habilidades.index(sabotaje) + 1, objetivo=defensor.nombre)
